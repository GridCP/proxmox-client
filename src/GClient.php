...
use GridCP\Proxmox_Client\VM\App\Service\ResetVM;
use GridCP\Proxmox_Client\VM\App\Service\ResizeVMDisk;
use GridCP\Proxmox_Client\VM\App\Service\ResumeVMinNode;
...
class GClient
{
    ...
    public function login(): LoginResponse
    {
        ...
        $response = $login->__invoke();

        if (is_null($response->getCookies())) {
            throw new AuthFailedException();
        }

        $this->cookiesPVE = $this->setCookiesFromResponse($response);
    ...
    public function getTaskStatusVM(string $node, string $upid): array|AuthFailedException|GetTaskStatusVMException|null
    {
        try {
            if (!isset($this->cookiesPVE)) {
                return new AuthFailedException('Auth failed!!!');
    ...
    /**
     * @param \GridCP\Proxmox_Client\Commons\Domain\Exceptions\AuthFailedException|\GridCP\Proxmox_Client\Commons\Domain\Exceptions\HostUnreachableException|\GridCP\Proxmox_Client\Auth\Domain\Responses\LoginResponse|null $result
     * @return \GridCP\Proxmox_Client\Commons\Domain\Entities\CookiesPVE
     */
    public function setCookiesFromResponse(AuthFailedException|HostUnreachableException|LoginResponse|null $result): CookiesPVE
    {
        return new CookiesPVE($result->getCSRFPreventionToken(), $result->getCookies(), $result->getTicket());
    }
}
